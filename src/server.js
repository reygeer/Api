...
// Servir el index.html y archivos estáticos
app.use(express.static(__dirname));

// Almacén de salas
let salas = {}; 
// Estructura: { "codigoSala": { jugadores: { socketId: {nombre, cartasSeleccionadas: []} } } }
...
io.on("connection", (socket) => {
  ...
  socket.on("unirseSala", ({ nombre, sala }) => {
    if (!salas[sala]) {
      socket.emit("errorSala", `La sala ${sala} no existe`);
      return;
    }
